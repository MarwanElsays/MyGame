...
class PauseState(GameState):
    ...
    def intiallize(self):
        ...
        self.x, self.y = 20, 0
        self.scale = (3, 3)
        self.knight_idle_sheet = SpriteSheet(
            "knight/knightIdle.png", "jsonImages/knight.json", "#71664f", self.scale
        )
        self.knight_attack_sheet = SpriteSheet(
            "knight/knightAttack.png", "jsonImages/knight.json", "#71664f", self.scale
        )
        self.animations = {
            "idle": Animation(
                self.knight_idle_sheet.get_sprite_images("knightIdle"), 6, "idle"
            ),
            "attack": Animation(
                self.knight_attack_sheet.get_sprite_images("knightAttack"), 6, "attack"
            ),
        }

        # self.pos = [(self.x,self.y+i*100) for i in range (len(self.knightAttack))]
        self.curr_animation = self.animations["idle"]
        self.image = self.curr_animation.get_image()
    ...
    def update(self):
        ...
        new_img = self.curr_animation.get_image()
    ...
    def render(self):
        ...
        if self.flip:
            img_offset = (
                -(self.image[0].get_rect().w - 35) + self.image[1] * self.scale[0]
            )
        else:
            img_offset = -self.image[1] * self.scale[0]

        self.screen.blit(
            pygame.transform.flip(self.image[0], self.flip, False),
        ...
        pygame.draw.line(
            self.screen,
            "#FF0000",
            (0, self.rect.y + self.rect.h),
            (1200, self.rect.y + self.rect.h),
        )
