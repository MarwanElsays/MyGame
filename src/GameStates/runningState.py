...
class RunningState(GameState):
    ...
    def intiallize(self, character="player"):
        ...
        self.controlProjectile = ProjectileController(
            self.map, self.enemy, self.explosions
        )
        ...
        self.tree = pygame.image.load(
            "Assets/images/tiles/Trees/Tree1.png"
        ).convert_alpha()
    ...
    def run(self, timeDelta):
        ...
        self.offset[0] += (
            self.player.get_rect().centerx - SCREEN_WIDTH / 4 - self.offset[0]
        ) / 15
        if abs(self.player.get_rect().centery - SCREEN_HEIGHT / 4 - self.offset[1]) > 4:
            self.offset[1] += (
                self.player.get_rect().centery - SCREEN_HEIGHT / 4 - self.offset[1]
            ) / 15
        ...
        self.display.blit(
            healthTextSurface, healthTextSurface.get_rect(topleft=(265, 3))
        )
        pygame.draw.rect(self.display, (255, 255, 200), pygame.Rect(295, 5, 100, 8))
        pygame.draw.rect(
            self.display, (255, 0, 0), pygame.Rect(295, 5, self.player.get_health(), 8)
        )
        ...
        for events in pygame.event.get():
            ...
            if events.type == pygame.MOUSEBUTTONDOWN:
                self.game_statesManager.getPauseState().intiallize()
                self.game_statesManager.setGameState(
                    self.game_statesManager.getPauseState()
                )
        ...
        self.screen.blit(
            pygame.transform.scale(
                self.display,
                (
                    SCREENSCALE * self.screen.get_size()[0],
                    SCREENSCALE * self.screen.get_size()[1],
                ),
            ),
            (0, 0),
        )
